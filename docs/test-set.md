...
6. **Negative net savings** — User withdrew from savings this month. Frank should acknowledge this distinctly from simply "low savings." - **PASS** (Frank acknowledged zero/negative savings directly)

7. **High income, low spending** — Large gap between income and spend. Frank should recognize this as a strong month, not roast generically. - **PASS** (Frank recognized strong month, specific food advice)
...
13. **Multiple months, improving trend** — Spending has decreased month over month across 3+ months. Frank should be able to reference positive trend direction. - **PASS** (graph shows trend, and Frank mentions it in passing)

14. **Multiple months, worsening trend** — Spending has increased steadily. Frank should call out the trend, not just the single month. - **PASS** (existing data shows upward trend, Frank references it)

15. **Category name edge case** — A less common category present (e.g. "Medical" or "Subscriptions"). Frank should reference it naturally, not awkwardly. - **PASS** (medical referenced naturally by name and amount)

16. **Income missing or zero** — Edge case where income field is 0 (possibly unemployed user, or data entry skipped). Frank's response should not divide-by-zero or break when calculating ratios involving income. - **PASS** (no crash, Frank addressed zero income directly)
